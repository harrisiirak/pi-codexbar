...
test('formatUsageFooter: success shows provider, plan, session, weekly, reset', () => {
...
test('formatUsageFooter: error entry shows error message', () => {
  const state: UsageState = {
...
test('formatUsageFooter: monthly shown when tertiary exists', () => {
  const state: UsageState = {
...
test('formatUsageFooter returns ANSI-colored output', () => {
  const state: UsageState = {
    selectedProvider: 'claude',
    entries: [{
      providerId: 'claude',
